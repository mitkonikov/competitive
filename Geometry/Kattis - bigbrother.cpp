...
// returns twice the polygon area
long double polygonArea(vector<Point>& v) {
...
// Actual algorithm
long double hp_intersect(vector<Halfplane>& H) { 
...
int main() {
    ...
    for (int i = n; i >= 0; i--) {
        halfplanes.push_back(Halfplane(points[i%n], points[(i-1+n)%n]));
    }

    cout << fixed << setprecision(15) << hp_intersect(halfplanes) << endl;
}